...
function RepositoriesPage({ navigate }: { navigate: (route: string) => void }) {
  ...
  return (
    <>
      <PageHeader eyebrow="Assets · SCM" title="Repositories" description="GitHub and GitLab codebases synchronized through governed provider connections." actions={<div className="scm-actions"><button className="secondary-button" disabled={!providers.gitlab} onClick={() => connect("gitlab")}>Connect GitLab</button><button className="primary-button" disabled={!providers.github} onClick={() => connect("github")}>Install GitHub App</button></div>} />
