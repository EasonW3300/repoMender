...
func TestRealAgentComposeCancellation(t *testing.T) {
	if os.Getenv("REPOMENDER_AC_REAL_SMOKE") != "1" {
		t.Skip("set REPOMENDER_AC_REAL_SMOKE=1 to run against a real AC daemon")
	}
	baseURL := os.Getenv("REPOMENDER_AC_BASE_URL")
	...
	client, err := New(Config{BaseURL: baseURL, RequestTimeout: 15 * time.Second, RequiredDriver: "docker"})
	if err != nil {
		t.Fatal(err)
	}
	...
	defer cancel()
	run, err := client.Start(ctx, execution.Request{
		CorrelationID: "m3-real-cancel-" + time.Now().UTC().Format("20060102T150405Z"),
		ProjectID:     projectID, AgentName: "codex", Repository: repository,
		CommitSHA: commit, Prompt: "Wait for further instructions without modifying the repository.",
		Timeout: 5 * time.Minute,
		Policy: execution.ResourcePolicy{
			Driver: "docker", Cleanup: "remove",
		},
	...
	if err := client.Cancel(ctx, run.ID, "M3 real cancellation smoke"); err != nil {
		t.Fatalf("Cancel() error = %v", err)
	}
	events, errs := client.Events(ctx, run.ID, 0)
	terminal := false
	for event := range events {
		t.Logf("event sequence=%d kind=%s message=%s", event.Sequence, event.Kind, event.Message)
		terminal = terminal || event.Terminal
	}
	if err := <-errs; err != nil {
		t.Fatalf("Events() error = %v", err)
	...
	if !terminal {
		t.Fatal("cancelled run omitted terminal event")
	}
	if _, err := client.Result(ctx, run.ID); !errors.Is(err, ErrCancelled) {
